...
import type { Category, Shop as ShopBase, WorkHours } from "@/lib/type"
...
export default function ShopsPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      setLoading(true)
      // جلب المتاجر
      const { data: shops, error: shopsError } = await supabase.from("shops").select("*, profiles(full_name)")
      // جلب التصنيفات
      ...
      if (!shopsError && shops && cats) {
        ...
        const shopsWithCount = shops.map((shop) => {
          const count = products ? products.filter((p) => p.shop === shop.id).length : 0
          return {
            ...shop,
            categoryTitle: cats.find((cat) => cat.id === shop.category_id)?.title || "بدون تصنيف",
            productsCount: count,
          }
        })
  ...
  const filteredAndSortedShops = useMemo(() => {
    ...
    switch (sortBy) {
      case "rating":
        ...
        break
      case "products":
        filtered.sort((a: any, b: any) => (b.productsCount ?? 0) - (a.productsCount ?? 0))
        break
      case "alphabetical":
  ...
  const handleVisitShop = async (shopId: string) => {
    ...
    // يمكنك هنا إعادة جلب بيانات المتجر إذا أردت تحديث العدد في الواجهة
  }
                ...
                <CardContent className="p-0">
                  ...
                  <div className="p-6 pt-4 flex flex-col gap-3">
                    <div className="flex items-center justify-between gap-2 mb-2">
                      <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{shop.shop_name}</h3>
                    ...
                    {/* الوصف */}
                    <p className="text-sm text-gray-600 dark:text-gray-400 mb-1">{shop.shop_desc}</p>
