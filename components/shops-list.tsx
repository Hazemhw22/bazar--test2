        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
            {paginatedShops.map((shop) => (
              <Link href={`/shops/${shop.id}`} key={shop.id} className="group">
                <Card
                  className="overflow-hidden hover:shadow-lg transition-all duration-300 rounded-2xl bg-card cursor-pointer"
                >
